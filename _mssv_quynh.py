...
class QLearning():
    ...
    def learn_state(state, winner):
        ...
        if QLearning.tag in stateStr:
            ...
            QLearning.values[QLearning.prev_stateStr] = prev + \
                QLearning.alpha*(R + QLearning.discount_factor*curr - prev)
...
class Dlearning():
    ...
    def calc_target(state, winner):
        ...
        if Dlearning.tag in state:
            ...
            target = np.array(v_s + Dlearning.alpha *
                              (R + 0.8 * v_s_tag - v_s))
    ...
    def train_model(target, epochs):
        ...
        if target is not None:
            Dlearning.value_model.fit(
                X_train, target, epochs=epochs, verbose=0)
